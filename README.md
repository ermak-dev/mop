...
* Инвентарь ansible с ролями-группами: `server` (сервер), `puppet` (узлы
  пула), `master` (управляющая машина) и `hypervisor` (узел Proxmox, чьи
  тела папетов — контейнеры LXC). Заводится в корне проекта из
  `inventory.yaml.example`, в git не попадает.
...
Окружение контейнера печётся в образ командой `mop driver build <origin>`.
...
3. Развернуть пул:
   ...
   печётся отдельно: `mop driver build <git-origin>`.
...
mop driver build <origin>  испечь образ шарда на гипервизорах (LXC)
