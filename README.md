...
* `~/.env` — общий файл секретов машины; `mop` берёт оттуда ключи
  LLM-провайдеров и **вывозит на узлы только те, что называет хоть один профиль
  в `LLM_PROFILES`**. Остальное (gitlab, smtp, платёжки) пул не видит.
