...
def distribute(files):
    ...
    results = {}
    _push_via_workers(script, results)
...
def _push_via_workers(script, results):
    """Узлы с живым воркером — через exec в его аллокацию."""
    try:
        jobs = seats.jobs()
    except Exception:
        return
