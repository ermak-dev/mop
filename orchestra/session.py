...
импорта из пакета orchestra. Он работает в двух режимах:
  ...
  * исходником, уехавшим внутрь аллокации Nomad: сокет сиденья host-local, с
...
def live_session_for_cwd(cwd):
    ...
    Файлов на один каталог накапливается много (у сиденья wk-rugent-1 их было
...
def resolve(target):
    """Куда писать. Каталог -> живая сессия в нём (так адресуют сиденье пула),
...
def envelope(body, from_addr=None, from_name=None, mode="bypass"):
    ...
    Сиденье с --dangerously-skip-permissions НЕ отдаёт модели сообщение от
    отправителя, который не заявил свой режим: оно кладёт его в held-очередь и
    ждёт живого человека. Для воркера это неотличимо от потери сообщения.
