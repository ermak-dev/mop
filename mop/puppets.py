...
def _clone_veto(clone):
    ...
    cur = clone.get("cur")
    return f"idle: {cur} ({what})" if cur else f"idle ({what})"
...
def puppet_state(f):
    ...
    state = _verdict(f)
    if not is_free(state):
        return state
    return _clone_veto(f.get("clone")) or state


def _verdict(f):
    """Активность папета по экрану и файлу сессии. Про клон см. puppet_state."""
