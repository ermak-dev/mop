...
def config():
    """{url, user, password} — раскатывается ansible'ом вместе с агентом.

    Отдельный файл, а не переменные окружения: тем же путём уже лежит токен
    Nomad, и искать креды в одном месте дешевле, чем помнить два соглашения."""
    try:
