...
DEFAULTS = {
    ...
    # Проекты этого пула, чьи манифесты прогон применяется на host-узлах
    # (#25). Это знание УСТАНОВКИ — какие проекты в её пуле; identity шарда
    # остаётся у basename origin, таблицы имён это не заводит. Пусто — на
    # host-узлах применяется только общее (MOP_BODY_EXTRA).
    "MOP_SHARD_ORIGINS": "",
...
OPTIONAL = {
    ...
    #
    # Пути, а не имена ролей: setup.yaml — ПРИМЕР установки в корне, и
    # ролевое дерево ему ни к чему. Ровно поэтому же настройку не отдали
    # ПРОЕКТУ в #13: путь это «исполни вот этот файл», то есть исполнение
    # на узле.
    #
    # Настройкой, а не строкой в deploy/: граница продукта проходит ровно
    # здесь. Нужное КОНКРЕТНОМУ проекту живёт в его .mop/ и
    # подключается оттуда, иначе deploy/ обрастает чужими тулчейнами.
    #
