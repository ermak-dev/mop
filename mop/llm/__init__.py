...
Профиль меняет ровно одно — КУДА слейв ходит за токенами: эндпоинт
...
собранные SL_LLM_ENV/SL_LLM_KEY_VAR/SL_LLM_AUTH_VAR и самого профиля не
...
Контракт модуля:
    ...
    ENV       статические переменные сессии слейва, dict[str, str]

Контракт проверяется громко и с именем файла: KEY уезжает в sed-шаблон
врапера на узле, и кривое имя там молча совпадёт нигде — слейв умрёт с
...
def contract(name, mod):
    ...
    (tests/llm.py): ошибка контракта обязана находиться до живых слейвов.
    """
