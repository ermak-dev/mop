...
контракт claude, описанный в CHANNEL.md: где лежит файл сессии, что в нём, где
...
def find(target):
    ...
    if len(hits) > 1:
        ...
        raise LookupError(f"под '{target}' подходит несколько сессий: {names} — уточни pid")
    return hits[0]
...
#
# Про адрес отправителя намеренно молчим. Раньше причина была в том, что
# отправитель жил внутри alloc exec и умирал сразу после доставки; теперь
# доставляет долгоживущий агент узла, но uds-адреса он всё равно не обещает —
# ответ надо слать через шину, а не в сокет, который агент не слушает.
REPLY_HINT = (
    "[канал mop] Доставлено сокет-каналом пула. Отправитель ответа не ждёт "
    "и обратного адреса не даёт. Если нужно с кем-то связаться — используй "
